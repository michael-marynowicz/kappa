...
public class JiraIssueRepositoryImpl implements JiraIssueRepository {
    ...
    private List<SprintIssue> fetchFromJiraApi() {
        WebClient client = WebClient.builder()
                ...
                // *all fields can return responses >256KB — raise the codec buffer limit
        ...
        JiraApiResponse.SearchResult result = client.get()
                .uri(uriBuilder -> {
                    var builder = uriBuilder
                            ...
                            // *all requests every field so we can log which customfield contains story points
    ...
    private String buildJqlFilter() {
        ...
        // Exclude sub-tasks and Epics — we only want Stories, Bugs, Tasks
        conditions.add("issuetype not in subTaskIssueTypes() AND issuetype != Epic");
